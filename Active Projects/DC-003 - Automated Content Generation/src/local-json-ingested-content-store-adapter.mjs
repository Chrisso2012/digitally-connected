// DC-003-I030 — Local JSON Storage Adapter for Ingested Content: one file
// per record at `<storageDir>/<ingested_content_id>.json`. Byte-for-byte
// mirror of local-json-engineering-work-order-store-adapter.mjs — same
...
export function createLocalJsonIngestedContentStoreAdapter({ storageDir }) {
  function finalPath(identifier) {
    return path.join(storageDir, `${identifier}${EXTENSION}`);
  }

  return {
    name: "local-json-ingested-content-store",
