// DC-003-I032.10.1 — Local JSON Storage Adapter for Carousel Content
// Packages: one file per record at
// `<storageDir>/<carousel_content_package_id>.json`. Byte-for-byte
// mirror of local-json-editorial-package-store-adapter.mjs — same
// atomic-write strategy (temp file in the same directory, read-back-
// verify, then rename), same "storageDir is always an explicit
...
export function createLocalJsonCarouselContentPackageStoreAdapter({ storageDir }) {
  function finalPath(identifier) {
    return path.join(storageDir, `${identifier}${EXTENSION}`);
  }

  return {
    name: "local-json-carousel-content-package-store",
