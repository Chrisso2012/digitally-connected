// DC-003-I029 — Local JSON Storage Adapter for Engineering Work Orders:
// one file per record at `<storageDir>/<work_order_id>.json`. Byte-for-byte
// mirror of local-json-social-analytics-store-adapter.mjs (I028) — same
// atomic-write strategy (temp file in the same directory, read-back-verify,
// then rename), same "storageDir is always an explicit constructor
// argument" rule.
...
export function createLocalJsonEngineeringWorkOrderStoreAdapter({ storageDir }) {
  function finalPath(identifier) {
    return path.join(storageDir, `${identifier}${EXTENSION}`);
  }

  return {
    name: "local-json-engineering-work-order-store",
