// DC-003-I031 — Local JSON Storage Adapter for Editorial Packages: one
// file per record at `<storageDir>/<editorial_package_id>.json`.
// Byte-for-byte mirror of local-json-ingested-content-store-adapter.mjs —
...
export function createLocalJsonEditorialPackageStoreAdapter({ storageDir }) {
  function finalPath(identifier) {
    return path.join(storageDir, `${identifier}${EXTENSION}`);
  }

  return {
    name: "local-json-editorial-package-store",
